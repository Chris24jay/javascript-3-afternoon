...
*/
// I want to go over the array and do some math. I have to multiply the price and tax propeties
...
let bobFilter = purchases.filter((val, ) => {
  if(val.owner === "Bob"){
    return val
  }
})
...
let bobPrice = bobFilter.map((val,) => {
   return val.price
  {}
})
...
let bobsTotal = bobPrice.reduce((total, val, ind) => {
  return total + val
})
